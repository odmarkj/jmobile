window['$'] = window['jMobile'] = {
	...
	geoTimeout: 7500,
	detect: function(options, callback_success, callback_error, callback_watch_success, callback_watch_error){
		if (navigator.geolocation) {
		  // google chrome times out starting with the call to getCurrentPosition (rather than after permission is granted)
		  if (navigator.userAgent.indexOf('Chrome') != -1)
		    jMobile.geoTimeout = 60000;
		  jMobile.geolocationFailFast = true;
		  navigator.geolocation.getCurrentPosition(
		    callback_success,
		    callback_error,
		    {maximumAge:600000,timeout:jMobile.geoTimeout}
		  );

		  setInterval('jMobile.geolocationFailFast = false;', 100);

			if(typeof options === "object" && typeof options !== "undefined" && typeof options.watch !== "undefined" && options.watch == true){
				jMobile.watch_id = navigator.geolocation.watchPosition(
			    callback_watch_success, 
			    callback_watch_error,
			    {maximumAge:600000,timeout:jMobile.geoTimeout,enableHighAccuracy:true}
			  );
			}
		}else{
			callback_error({code:1,message:"Browser does not support geolocation."});
		}
	},
	clear_watch: function(){
		navigator.geolocation.clearWatch(jMobile.watch_id);
	},
	select: (function(){
		var win = window, 	
